...
ship-events-to-s3:
	kubectl run --rm --attach ship-events \
		...
		-- \
		bash -c 'yum install --assumeyes --quiet postgresql && psql -c "copy (select json_agg(events) from events) To STDOUT;" && aws s3 cp "./events.json" "s3://sciety-data-extractions/events.json"'
